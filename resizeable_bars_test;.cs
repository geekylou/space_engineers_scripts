...
public void Main(string argument, UpdateType updateSource)
{
    ...
    foreach(TextPanel panel in text_panels)
    {
        ...
        DrawBar("IconHydrogen",0.25,ref frame,new Vector2(0,10),panel.viewport);
        DrawBar("IconOxygen",0.50,ref frame,new Vector2(0,60),panel.viewport);
        DrawBar("IconEnergy",0.75,ref frame,new Vector2(0,110),panel.viewport);
        DrawBar("MyObjectBuilder_Ingot/Uranium",1.0,ref frame,new Vector2(0,160),panel.viewport);
...
public void FindTextPanels()
{
    ...
    {
        String[] lines = text_panel.CustomData.Split('\n');
        foreach(String line in lines)
        {
            ...
            if (line_vals.Length>1 && line_vals[1].Equals(group_name) && line_vals[0].Equals(SCRIPT_PREFIX))
            {
                ...
                Echo(DBG);
                TextPanel panel = new TextPanel();
                panel.panel = text_panel.GetSurface(Convert.ToInt32(line_vals[2]));
                panel.include_battery = Convert.ToInt32(line_vals[3]);
                panel.viewport = new RectangleF(
                    (panel.panel.TextureSize - panel.panel.SurfaceSize) / 3f,
                    panel.panel.SurfaceSize
                );
                Echo(panel.viewport.ToString());
                text_panels.Add(panel);
            }
        }
    }
...
// Drawing Sprites
public void DrawBar(string type,double ratio, ref MySpriteDrawFrame frame,Vector2 base_position,RectangleF viewport)
{
